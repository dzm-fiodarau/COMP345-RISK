...
const std::vector<Continent*>& Map::getContinents() const { //debug
    return continents;
}
    ...
    for (const auto& continent : continents) {
        ...
        for (const auto& territory : continent->getTerritories()) {
            ...
            for (size_t i = 0; i < adjacentTerritories.size(); ++i) {
                ...
                if (i != adjacentTerritories.size() - 1) { // If not the last neighbor
                    std::cout << ", ";
                }
            }
            std::cout << std::endl;
...
Map MapLoader::load() {
    ...
    std::cout << "Loaded " << map.getNumTerritories() << " territories." << std::endl << std::endl;
